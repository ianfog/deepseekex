...
把 [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) 打包成桌面客户端（Windows / macOS）。
...
- **壳界面**：Endfield 现场工程系统视觉语言（`endfield` 风格族 · 复杂档）——炭墨 `#191919`
  基底 + 米白 `#f2f2f0` 文本 + 荧光黄 `#fffa00` 唯一信号色 + 在线绿 `#00ffa2`（仅在线/已验证态）+
  切角楔形按钮 + 校准刻度尺 + 四角定位框 + 大号编号 + 1px 发丝线。动效：顶栏信号轨黄色擦入、
  状态块呼吸、菱形旋转加载；`prefers-reduced-motion` 下全部关闭。
